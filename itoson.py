    ...
    while True:
        ...
        try:
            ...
            if len(phone) > 0 and element2 == "":
                ...
                personels_caount = int(personels_caount[:6])
...
####                else:
                cursor.execute(
                        "INSERT INTO crawler_companies (sector, name, phone, site, address, personels_caount, note, created_date, updated_date, city_id, fount_id, last_status_id, user_id, full_name, reminder, short_name) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                        data_tuple,
                )
                    ## databaseye kaydet
                connection.commit()
                logging.info("Veritabanına veri eklendi: %s %s", personels_caount, "başarılı")
...
####                wb.save("1.xlsx")
                ...
                logging.info("Veritabanına veri eklenirken bir hata oluştu:%s %s",personels_caount,"boş telefon numarası")

        except Exception as err:
            print("Hata:", err)
            logging.info("Veritabanına veri eklenirken bir hata oluştu:%s %s",personels_caount,"Hata")
